...
> [`docs/package_manager.md`](package_manager.md).
...
The full set is in [`SYNTAX.md`](SYNTAX.md) and the `lamc --help`
output.
...
- **First Lam program**: [`SYNTAX.md`](SYNTAX.md) — the complete
  language surface in one file.
- **Standard library**: [`stdlib.md`](stdlib.md) — every `lam*`
  module, every class, every method.
- **Adding third-party deps**: [`package_manager.md`](package_manager.md) —
...
- **Authoring a third-party library**:
  [`third_party_libraries.md`](third_party_libraries.md) — the
