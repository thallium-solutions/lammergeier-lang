...
def _run_case(path: Path) -> tuple[bool, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="lamstripe_test_") as tmp:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="lamstripe package tests")
    ap.add_argument("--live", action="store_true", help="include live Stripe tests")
    ap.add_argument("--verbose", "-v", action="store_true")
    args = ap.parse_args()

    cases = sorted((LIB_ROOT / "tests").glob("offline_*.lam"))
    if args.live:
        cases.append(LIB_ROOT / "tests" / "live_balance.lam")

    passed = 0
    failures: list[tuple[Path, str]] = []
    print(f"Running {len(cases)} lamstripe test(s)...\n")
    for case in cases:
    ...
    print(f"\nLamstripe results: {passed} passed, {len(failures)} failed, {len(cases)} total")
