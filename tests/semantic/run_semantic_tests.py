...
def _expectations(source: str) -> tuple[list[str], bool]:
    """Return (error_substrings, expect_pass)."""
    errors: list[str] = []
    expect_pass = False
    for line in source.split("\n"):
        ...
        if re.match(r'^\s*#\s*expect-pass\s*$', line):
            expect_pass = True
    return errors, expect_pass


def _run_case(lam_file: Path) -> tuple[bool, str]:
    source = lam_file.read_text(encoding="utf-8")
    expected_errors, expect_pass = _expectations(source)
    if not expected_errors and not expect_pass:
        return False, "no `# expect-error` or `# expect-pass` declared"
